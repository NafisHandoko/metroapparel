...
import {
  METRO_SITE_COMPANY_METADATA_KEY,
  isMetroSiteCompanySectionDefault,
  metroSiteCompanySchema,
  resolveMetroSiteContent,
  serializeMetroSiteCompany,
...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  res.status(200).json({
    ...
    is_default_template: isMetroSiteCompanySectionDefault(store.metadata),
  });
}

export async function POST(req: MedusaRequest, res: MedusaResponse) {
  const parsed = metroSiteCompanySchema.safeParse(req.body);
  if (!parsed.success) {
  ...
  const mergedMetadata = {
    ...(store.metadata ?? {}),
    [METRO_SITE_COMPANY_METADATA_KEY]: serializeMetroSiteCompany(parsed.data),
  };
  ...
  res.status(200).json({ ok: true, company: content.company });
}
