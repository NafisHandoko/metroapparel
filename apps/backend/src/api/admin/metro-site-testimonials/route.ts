...
import {
  METRO_SITE_TESTIMONIALS_METADATA_KEY,
  isMetroSiteTestimonialsSectionDefault,
  metroSiteTestimonialsPostSchema,
  resolveMetroSiteContent,
  serializeMetroSiteTestimonials,
...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  res.status(200).json({
    ...
    is_default_template: isMetroSiteTestimonialsSectionDefault(store.metadata),
  });
}

export async function POST(req: MedusaRequest, res: MedusaResponse) {
  const parsed = metroSiteTestimonialsPostSchema.safeParse(req.body);
  if (!parsed.success) {
  ...
  res.status(200).json({ ok: true, testimonials: content.testimonials });
}
