...
import {
  METRO_SITE_CLIENTS_METADATA_KEY,
  isMetroSiteClientsSectionDefault,
  metroSiteClientsPostSchema,
  resolveMetroSiteContent,
  serializeMetroSiteClients,
...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  res.status(200).json({
    ...
    is_default_template: isMetroSiteClientsSectionDefault(store.metadata),
  });
}

export async function POST(req: MedusaRequest, res: MedusaResponse) {
  const parsed = metroSiteClientsPostSchema.safeParse(req.body);
  if (!parsed.success) {
  ...
  const mergedMetadata = {
    ...(store.metadata ?? {}),
    [METRO_SITE_CLIENTS_METADATA_KEY]: serializeMetroSiteClients(
      parsed.data.clients,
    ),
  };
  ...
  res.status(200).json({ ok: true, clients: content.clients });
}
