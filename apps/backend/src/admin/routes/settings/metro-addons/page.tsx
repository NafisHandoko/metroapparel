...
function apiEntryToForm(e: ApiEntry): EntryForm {
  return {
    ...
    inputQuantity: e.input === "quantity",
    groupFabric: e.group === "fabric-extra",
    scope: e.scope,
...
function formToPayload(entries: EntryForm[]) {
  return entries.map((row) => {
    const price = Number.parseInt(row.price.replace(/\s/g, ""), 10);
    return {
      id: row.id.trim(),
      label: row.label.trim(),
      description: row.description.trim() || undefined,
      price: Number.isFinite(price) ? price : 0,
      input: row.inputQuantity ? ("quantity" as const) : undefined,
      group: row.groupFabric ? ("fabric-extra" as const) : undefined,
      scope: row.scope,
...
const MetroAddonsSettingsPage = () => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/admin/metro-addon-catalog", {
        credentials: "include",
      });
      ...
      console.error(e);
      toast.error("Gagal memuat pengaturan add-on.");
  ...
  const save = async () => {
    if (duplicateIds.size) {
      toast.error("Ada ID add-on yang dobel. Setiap ID harus unik.");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) throw new Error(await res.text());
      toast.success("Add-on disimpan. Toko akan memakai daftar ini.");
      await load();
    } catch (e) {
      console.error(e);
  ...
  return (
    <Container className="divide-y p-0">
      <div className="flex flex-col gap-2 px-6 py-4">
        <Heading level="h1">Add-on & biaya tambahan (Metro)</Heading>
        <Text size="small" className="text-ui-fg-subtle max-w-3xl">
          Atur harga opsi tambahan untuk konfigurator dan checkout. Secara
          default setiap add-on berlaku untuk semua produk. Gunakan &quot;Hanya
          produk terpilih&quot; atau &quot;Kecuali produk terpilih&quot; untuk
          membatasi per add-on. Penyimpanan memakai data toko di belakang layar
          — tidak perlu mengedit JSON.
        </Text>
      </div>
              ...
              >
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="flex flex-col gap-2">
                    <Label htmlFor={`id-${i}`}>ID (unik, tidak diubah setelah dipakai di pesanan)</Label>
                    <Input
                      id={`id-${i}`}
                      value={row.id}
                      onChange={(e) => patchRow(i, { id: e.target.value })}
                      placeholder="mis. 3d-logo"
                    />
                  ...
                  <div className="flex flex-col gap-2">
                    <Label htmlFor={`label-${i}`}>Nama tampilan</Label>
                    <Input
                      id={`label-${i}`}
                      value={row.label}
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="flex flex-col gap-2">
                    <Label htmlFor={`price-${i}`}>Harga (IDR)</Label>
                    <Input
                      id={`price-${i}`}
                      value={row.price}
                ...
                <div className="flex flex-col gap-2">
                  <Label htmlFor={`desc-${i}`}>Deskripsi (opsional)</Label>
                  <Textarea
                    id={`desc-${i}`}
                ...
                <div className="flex flex-wrap gap-6">
                  <label className="flex items-center gap-2 text-sm">
                    <input
                      type="checkbox"
                      checked={row.inputQuantity}
                      onChange={(e) =>
                        patchRow(i, { inputQuantity: e.target.checked })
                      }
                    />
                    Tipe kuantitas (contoh: up size per kelipatan)
                  </label>
                  <label className="flex items-center gap-2 text-sm">
                    <input
                      type="checkbox"
                      checked={row.groupFabric}
                      onChange={(e) =>
                        patchRow(i, { groupFabric: e.target.checked })
                      }
                    />
                    Grup kain emboss/jacquard (satu pilihan di configurator)
                  </label>
                </div>
...
export default MetroAddonsSettingsPage;
